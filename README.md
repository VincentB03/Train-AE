...
[experiments/verification.py](experiments/verification.py) uses PQMass (the `pqm` package) to statistically test whether images generated by the flow follow the same distribution as real held-out data, both in the autoencoder's latent space and in image space, with a real-vs-real calibration test as a sanity check on the setup itself.
